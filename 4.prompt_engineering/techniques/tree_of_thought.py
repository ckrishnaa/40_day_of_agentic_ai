...
tree_of_thought.py
------------------
Technique: Tree of Thought (ToT) Prompting

BEFORE: Asking the model to solve a complex, open-ended problem in a single pass.
AFTER:  Breaking the problem into distinct exploration and evaluation phases:
        1. Propose multiple different approaches or initial steps.
        2. Evaluate and critique those approaches to select the most robust one.
        3. Execute the chosen approach to generate the final output.

Sample input used for both:
    "I want to integrate an API Gateway for my microservices."
"""
...
def get_tree_of_thought(question: str = SAMPLE_INPUT, mode: str = "after") -> str:
    ...
    branches = run_prompt(build_propose_prompt(), {"question": question}, temperature=0.7)
    print(branches)

    print("\n--- ToT Step 2: Evaluating Branches ---")
    best_path_evaluation = run_prompt(build_evaluate_prompt(), {"question": question, "branches": branches})
    print(best_path_evaluation)

    print("\n--- ToT Step 3: Final Execution ---")
    final_output = run_prompt(build_final_prompt(), {"question": question, "best_path": best_path_evaluation})
    return final_output
